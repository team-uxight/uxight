...
- LLM 호출은 `LLMClient` 하나를 통해서만. 출력 채널은 `response_format`(기본) 또는 `emit_action` function tool **하나**(대안) — `tools` 목록에 다른 도구를 넣지 않는다. 제공사 서버 실행 도구(web_search · computer use · code interpreter)는 어떤 제공자에서도 요청하지 않는다 (`docs/agent-safety.md` §2.4). 제공자 전환은 `LLM_PROVIDER` env 로.
