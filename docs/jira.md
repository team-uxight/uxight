...
**누가 읽나:** 프로젝트 · 개발 경험이 많지 않은 팀원. 티켓만 읽고 무엇을 왜 해야 하는지 알 수 있어야 한다. PM 이 아니라 **리드가 하위 티켓을 같은 형식으로 만든다.**

**용어:** `docs/glossary.md` 의 말만 쓴다. PM 작업공간 표기(`D13` · `T17` · `O24` · "덱 11번" · "09/12 회의")는 티켓에 넣지 않는다 — 필요하면 문서 링크 하나로 대신한다.

**형식 (이 네 줄 이상 쓰지 않는다):**

```
왜: 한 줄 — 이게 없으면 무엇이 안 되나
할 일: 1~3개 — 동사로 시작
완료 기준: 무엇이 어디에 있으면 끝인가 (파일 · 화면 · PR · 수치)
참고: 문서 링크 1~2개
```

| 나쁜 예 | 좋은 예 |
| --- | --- |
| "D11' 기준 Taxonomy 4종 우선, T12 모델 분리 감안하여 threshold 산정" | "왜: 행동 로그에서 '문제 상황' 을 자동으로 골라내려면 기준이 필요하다. 할 일: 4가지 문제 유형별로 '이러면 문제' 규칙과 심각도(높음/중간/낮음) 기준을 정한다. 완료 기준: 규칙 표가 `docs/`에 있고 예시 로그 1개에 적용해 본 결과가 붙어 있다" |
| 배경 5문단 + 회의 인용 | 배경은 링크로. 티켓은 스크롤 없이 한 화면 |

**제목:** `파트 · 무엇을` (예: `AI · Persona 속성 형식 정하기`). 상태 · 날짜는 제목에 넣지 않는다 (필드가 있다).
**하위 티켓:** 리드가 파트 안에서 나눈다. PM 은 한 사람에게 몰리는지만 본다 (O22).
